...
|---|--------|---------------|-----------|--------|---------|
| UX-014 | **Refresh feedback (spinner + toast) + cache-aware recommendations Refresh** | The non-destructive Refresh reloaded silently with no feedback, so it looked broken; and the recommendations "↻ Refresh" force-re-ran Opus on every click, wasting tokens even with no new calls. Now: each view exposes `reload()`, the header Refresh shows a button spinner (min-450ms) + a "Refreshed" toast while reloading in place; the recs Refresh is cache-aware (re-synthesizes, and spends Opus, only when the agent's call count changed). | E1, E3 | ✅ | S-018 |
| UX-013 | **Unified centered load loader + non-destructive Refresh** | Reload showed a small top-left "Connecting…" spinner that jumped to a bigger centered "Loading agents…" (a scoped `.spinner` overrode `.spinner-lg`; the overview's `view-enter` transform trapped the fixed loader). Now one shared `.page-loader` (28px, viewport-centered) spans every phase — `index.html` boot spinner → Connecting… → Loading agents… — at the exact same size/spot (instrumented: 28 / cx 640 / cy 385). Refresh now re-fetches the **active view's** data in place (silent; keeps stale data on failure) instead of bouncing to the overview. | E1 | ✅ | S-016 |
| UX-012 | **Connections & Settings → corner icon + modal** | The full-width connections bar caused a load-time layout shift and dominated the header. Demoted to a compact header sliders icon (status dot) that opens a short modal with the same details. Emil-correct: center-origin scale, ease-out enter / faster exit, backdrop + Esc close, focus management, body-scroll lock, reduced-motion fade. | E1, R1.2 | ✅ | S-016 |
| UX-011 | **Emil craft pass v2 (full frontend)** | Filled the craft gaps the S-013 pass didn't reach: view-enter transitions on drill-down (agent/call), CallView KPI scorecard stagger, signal + native-drawer entrances, press feedback on the new filter toggles, and reduced-motion coverage for every new transform. Validated via computed-style E2E checks. | E1, ADR-0005 | ✅ | S-016 |
| UX-010 | **Lead facts + observability signals surfaced across the dashboard** | The two signals (`missed_opportunity` R2.3, `human_action_needed` R2.6) + lead facts were backend-only (S-015). Now: CallView "Lead & Outcome" panel (booking status + facts + both signals with reasons + `source` provenance badge + expandable native `extractedData` drawer); AgentView header tallies + per-row MO/HA badges + signal filters; Overview per-agent + location-wide counts. Wired to `GET /api/leads` (+`/:callId`); verified live against Postgres. | R2.3, R2.6, E1 | ✅ | S-016 |
