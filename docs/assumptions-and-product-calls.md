...
| **A-009** | Integration | OAuth `chooselocation` fails with **`error.noAppVersionIdFound` / "No integration found with the id"** until the app's **Auth config (scopes + Redirect URL) is completed AND saved** in the portal (that's what registers the integration/app version). Also requires being logged into the sandbox. | 🟡 | Observed on first authorize attempt (S-006). Verify fix by completing+saving Auth config, then re-running `/oauth/install`. | R1.2, A-007 |
