...
| **A-003** | Data | "Existing call transcripts" are available via the HighLevel Voice AI API in a parseable form (speaker turns + timestamps). | 🟢/🟡 | **List shape confirmed live (S-009):** `GET /voice-ai/dashboard/call-logs` → `{callLogs:[], totalRecords, traceId}`. **Transcript / per-call shape still 🟡** — sandbox has 0 calls; need one real Voice AI call to capture `Get Call Log` + transcript structure. | R2.1 |
